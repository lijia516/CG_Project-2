...
public:
    ...
    int longestAxis()	{
        ...
        double x_long = bmax[0] - bmin[0];
        double y_long = bmax[1] - bmin[1];
        double z_long = bmax[2] - bmin[2];
        
        double len = fmax(x_long, fmax(y_long, z_long));
        
        if (len == x_long) return 0;
        if (len == y_long) return 1;
        if (len == z_long) return 2;
